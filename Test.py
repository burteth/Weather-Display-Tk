...
def callback():
    ...
    with open('/Users/ethanburt/Desktop/Coding/Weather_Python/weather-display/Weather_IDs.txt', 'rt') as city_list:
        ...
        if (cities != []):
            ...
            range_label = Label(city_info_frame, background='#222222', font=(
                settings_font, 15), text="1 of " + str(len(cities)), fg='#00f3c3')
            range_label.pack(fill='y', side="top")

            left_arrow_image = Image.open(BytesIO(
                (get('https://cdn3.iconfinder.com/data/icons/line/36/arrow_left-512.png')).content))
            wpercent = (30 / float(left_arrow_image.size[0]))
            hsize = int((float(left_arrow_image.size[1]) * float(wpercent)))
            left_arrow_image = left_arrow_image.resize(
                (30, hsize), Image.ANTIALIAS)
            left_arrow_image = ImageTk.PhotoImage(left_arrow_image)
            left_arrow_label = Label(
                city_info_frame, image=left_arrow_image, background="#00f3c3")
            ...
            detailed_city_info_frame = Frame(
                city_info_frame, background='#222222')
            detailed_city_info_frame.pack(side='left', fill='y', pady=40)

            new_city_name = Label(detailed_city_info_frame, background='#222222', font=(
                settings_font, 15), text=str(cities[0][0]), fg='#00f3c3', width=25)
            new_city_name.pack(fill='x', padx=10)

            country_name = Label(detailed_city_info_frame, background='#222222', font=(
                settings_font, 15), text=str(cities[0][1]), fg='#00f3c3')
            country_name.pack(fill='x', padx=10)

            information = partial(set_city, {'id': str(
                cities[0][2]), 'name': str(cities[0][0])})
            set_city_button = Button(detailed_city_info_frame, font=(
                settings_font, 15), text="Get Weather", fg='#222222', command=information)
            set_city_button.pack(fill='x', padx=10)

            right_arrow_image = Image.open(BytesIO(
                (get('https://cdn3.iconfinder.com/data/icons/line/36/arrow_right-512.png')).content))
            wpercent = (30 / float(right_arrow_image.size[0]))
            hsize = int((float(right_arrow_image.size[1]) * float(wpercent)))
            right_arrow_image = right_arrow_image.resize(
                (30, hsize), Image.ANTIALIAS)

            #shift_right = partial(move_right, city_num)

            right_arrow_image = ImageTk.PhotoImage(right_arrow_image)
            right_arrow_label = Label(
                city_info_frame, image=right_arrow_image, background="#00f3c3")
            ...
            def change_label(i):
                new_information = partial(
                    set_city, {'id': str(cities[i][2]), 'name': str(cities[i][0])})
                ...
                range_label.config(
                    text=(str(i + 1) + " of " + str(len(cities))))
...
header_label = Label(header_frame, text="Settings", fg="#222222",
                     bg="#00f3c3", font=(settings_font, header_size), pady=10)
header_label.pack(fill="both", expand=True)

left_search_frame = Frame(self, background='#222222')
left_search_frame.pack(fill="both", expand=False,
                       side='left', padx=35, pady=10)

search_frame_label = Label(left_search_frame, background='#222222', font=(
    settings_font, header_size - 5), text="Change City", fg='#00f3c3', width=20)
search_frame_label.pack(fill='x', expand=False, side="top", pady=10)

search_box = Entry(left_search_frame, textvariable=text_imput,
                   font=(settings_font, 15))
search_box.pack(fill="x", expand=False, side='top', pady=5)

search_button = Button(left_search_frame, background='#222222', font=(
    settings_font, header_size - 10), text="Search", command=callback)
...
color_header_label = Label(color_pick_frame, background='#222222', font=(
    settings_font, header_size - 5), fg=gui_color1, width=20, text="Change Background Colors")
...
def draw():
    for i in range(1530):
        ...
        new_color = rgb_to_hex(
            (color_combination_draw[0], color_combination_draw[1], color_combination_draw[2]))
...
global color_combination_dict
color_combination_dict = [(255, 1, 0),
                          (255, 2, 0),
                          (255, 3, 0),
                          (255, 4, 0),
                          (255, 5, 0),
...
header_label = Label(header_frame, text="Settings", fg = "#222222", bg=gui_color1, font=(settings_font, header_size), pady=10)
header_label.pack(fill="both", expand=True)

left_search_frame = Frame(self, background = '#222222')
left_search_frame.pack(fill="both", expand=False, side='left', padx=35, pady=10)

search_frame_label = Label(left_search_frame, background = '#222222', font=(settings_font, header_size - 5 ), text="Change City", fg=gui_color1, width = 20)
search_frame_label.pack(fill='x', expand=False, side="top", pady=10)
